...
test('Click links should update filtering selections', () => {
  ...
  component.find('span.filter-link').at(0).simulate('click');

  expect(props.updateFilters).toHaveBeenCalledTimes(1);
  expect(props.updateFilters).toHaveBeenCalledWith({
    author: props.commits[0].author,
  });

  component.find('span.filter-link').at(1).simulate('click');

  expect(props.updateFilters).toHaveBeenCalledTimes(2);
  expect(props.updateFilters).toHaveBeenCalledWith({
    repository: props.commits[0].repository.id,
  });
});
